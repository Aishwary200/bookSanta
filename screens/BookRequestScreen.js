...
export default class BookRequestScreen extends Component {
        ...
        db.collection('users').where('email_id', '==', this.state.userId)
            .onSnapshot(querySnapshot => {
                querySnapshot.forEach(doc => {
                    this.setState({
                        IsBookRequestActive: doc.data().IsBookRequestActive, userDocId: doc.id
                    })
                })
            })
    }
    ...
    updateBookRequestStatus = () => {
        // updating book status after receiving book
    ...
    sendNotification = () => {
        ...
        db.collection('users').where('email_id', '==', this.state.userId).get().then((snapshot) => {
            snapshot.forEach((doc) => {
                ...
                var lastName = doc.data().last_name // to get the donor id and book name 
        ...
        if (this.state.IsBookRequestActive === true) {
            ...
            return (
                <View style={{ flex: 1, justifyContent: 'center' }}>
                    ...
                    <View style={{ borderColor: "orange", borderWidth: 2, justifyContent: 'center', alignItems: 'center', padding: 10, margin: 10 }}>
                        <Text> Book Status </Text> <Text>{this.state.bookStatus}</Text> </View>
